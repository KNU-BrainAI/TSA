...
with tf.device('/GPU:0'):
    ...
    for i in range(33,34):
        ...
        for train_index, test_index in kf.split(X):
            ...
            #X_train = np.concatenate((X_train,X_da), axis= 0)
            #Y_train = np.concatenate((Y_train, Y_da), axis=0)
            ############################# EEGNet portion ##################################
            ...
            plt.figure()
            plot_confusion_matrix(preds, Y_test.argmax(axis = -1), names, title = 'Newmodel-S14')
            plt.show()
