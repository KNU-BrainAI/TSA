# -*- coding: utf-8 -*-
"""
Created on Thu Jul 22 15:26:14 2021
...
save_dir = 'C:/Users/PC/Desktop/TSA_result/EEGNet_Within/1_split/'
result_txt = 'EEGNet_1_split.txt'
...
for i in range(1,17):
    ...
    for filename in glob.glob('C:/Users/PC/Desktop/matlab/dataset/TSA_Raw/seq_sub'+str(i)+'000/*_fr.set'):
        for tmin,tmax in zip(tmin_list,tmax_list):
            ...
            epochs = mne.Epochs(eeglab_raw, events_from_annot, event_id, float(tmin), float(tmax),baseline = None)
    ...
    X=np.array(X)
    Y=np.array(Y)
    print("total_X size",len(X))
    print("total_Y size",len(Y))
    #samples = 3sec * 512Hz sampling rate
    kernels, chans, samples = 1, 64, 512
