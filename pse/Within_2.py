# -*- coding: utf-8 -*-
"""
Created on Tue Jul 27 21:01:30 2021
...
# GPU allocation
save_dir = 'C:/Users/PC/Desktop/SSSEP/result/within_2/DCNN/'
...
for i in range(1,17):
    ...
    files = fnmatch.filter(os.listdir(data_path),'seq_sub'+str(i)+'S000R'+'*.set')
    print(files)
    for file in files:
        print(file)
    ...
    datum = X[20:100]
    label = Y[20:100]
    #datum = np.concatenate([X[0:20], X[40:100]])
    #label = np.concatenate([Y[0:20], Y[40:100]])
    datum_test = X[0:20]
    label_test = Y[0:20]
        
    X_train.extend(datum)
    Y_train.extend(label)
    X_test.extend(datum_test)
...
print("Y_validate : ", len(Y_validate))
...
trn_loader = data_utils.DataLoader(trn, batch_size=128, shuffle=True)
...
testmodel = model
savepath = 'C:/Users/PC/Desktop/SSSEP/result/within_2/DCNN/testmodel.pth'
...
for epoch in range(num_epochs):  # epoch
    ...
    model.load_state_dict(torch.load('C:/Users/PC/Desktop/SSSEP/result/within_2/DCNN/testmodel.pth'))
...
plt.savefig(save_dir+'img/fold'+str(4)+'.png', dpi=300)
plt.show()
